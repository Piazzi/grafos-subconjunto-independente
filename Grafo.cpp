...
vector<No*> Grafo::getPorcentagem(vector<No*> candidatosOrdenadosPeloGrau, float alfa)  /// a cardinalidade da lista retornada � controlada pelo Alpha:
{
    ///     Alpha == 1: retorna apenas o melhor (guloso)
    int tamanhoCandidatos = candidatosOrdenadosPeloGrau.size();                         ///     Alpha == 0: retorna todos elementos (sele��o aleat�ria)
...
float Grafo::calculaQi(int atualMaiorCardinalidade, float mediaSolucoes)
{
    ...
    float Qi = pow(f/Ai, 10);   ///Par�metro de amplifica��o == 10
